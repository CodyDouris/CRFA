...
function renderSelectedArea(areaDisplay, areaAverage, geographyChange) {
    ...
    if (areaDisplay.data === areaAverage) {
        ...
        d3.select("#description").html(
            ...
            "</span> rate for <span class='clientLabel'>" +
        ...
        if (areaDisplay.data && !isNaN(areaDisplay.data[asrCrudeToggle])) {
            if (areaDisplay.data[asrCrudeToggle] >= areaAverage["Lower_CI_" + significantASRCrudeToggle] && areaDisplay.data[asrCrudeToggle] <= areaAverage["Upper_CI_" + significantASRCrudeToggle])
                significanceFlag = "not statistically different from the ";
            ...
            d3.select("#description").html(
                ...
                d3.select("#Sex").property("value") +
                "</span>  in <span class='clientLabel areaName'>" +
                //Extra code to ensure province in includes with health regions and CMAs
                capitalize(areaDisplay.data.Geo_Label) + (geographyChange !== "province/territory" ? 
                ", " + topojsonObjects["province/territory"].filter(function(item){ return item.properties.PRUID === areaDisplay.properties.PRUID.split("/")[0] })[0].properties.PREABBR : "") + //add the break in middle for ottawa/gatineau
                "</span> is <span class='clientLabel'>" +
                Number(areaDisplay.data[asrCrudeToggle]).toFixed(1) +
...
function renderMap(geographyChange) {
            ...
            tempG
                ...
                .style("fill", function(d) {
                    currentArea.forEach(function(area) {
                        if (area.properties.PRUID === d.properties.PRUID && area.properties.PRUID !== undefined)
                            d.data = area.data;
    ...
    function createMap() {
        ...
        let legend = d3.select("#map")
            .append("g")
            .attr('id', 'legend')
            .attr("transform", "translate(" + (margin.left + 370) + "," + (margin.top - 30) + ")");
        ...
        let canadaCircle = d3.select("#map")
            ...
            .attr("transform", "translate(" + (margin.left + 450) + "," + (margin.top + 200) + ")");
        ...
        if (geographyChange === "Census Metropolitan Areas/Large Census Agglomerations") {
            ...
            d3.select("#CMANotes").html("<li>CMAs/large CAs definition: This map includes all Census Metropolitan Areas (CMAs), large Census Agglomerations (CAs) plus the city of Iqaluit.  Large CAs includes CAs with census tracts  and one CA with no census tracts (i.e. Fredericton) for which sufficient data was available to produce reliable estimates </li>");
        ...
        if (ecumeneToggle) {
            ...
            mapSVG
                ...
                .style("fill", function(d) {
                    currentArea.forEach(function(area) {
                        let tempPRUID = area.properties.PRUID;
                        if(area.properties.PRUID !== undefined && (area.properties.HR_UID !== undefined || area.properties.CMA !== undefined)) //quick change on first change to ecumenes 
                            area.properties.PRUID = undefined;
                        if (area.properties.PRUID === d.properties.PRUID && area.properties.PRUID !== undefined)
                            d.data = area.data;
                        else if (area.properties.HR_UID === d.properties.HR_UID && area.properties.HR_UID !== undefined)
                            d.data = area.data;
                        ...
                        area.properties.PRUID = tempPRUID;
                    });
                    return returnColor(d);
                })
                .style("stroke", "#333333")
